...
# solving ODEs epsilon * u'' - x * u = 0, u(-1) = Ai(-(1/epsilon)^(1/3)), u(1) = Ai((1/epsilon)^(1/3)), where the exact solution u = Ai((1/epsilon)^(1/3) * x)
...
for i in eachindex(nvec)
    ...
    ben = @benchmark GSBSPG_Chebyshev($(T), $(airy_coeffs_GSBSPG), $(airy_Wtrial), $(airy_v_GSBSPG), $(fc))
    time_solve[i, 2] = minimum(ben).time / 1e9

    # GSBSPG (numerical integration)
    ben = @benchmark GSBSPG_Chebyshev_NI($(T), $(airy_coeffs_GSBSPG), $(airy_Wtrial), $(airy_v_GSBSPG), $(fc))
...
for i in eachindex(nvec)
    ...
    # GSBSPG (recurrence)
    fc = Chebyshev_rhs_NI(T, f, n-2, n, 2)
    L1, fc1 = GSBSPG_Chebyshev(T, airy_coeffs_GSBSPG, airy_Wtrial, airy_v_GSBSPG, copy(fc))
    u1 = airy_Wtrial * (L1 \ fc1)
    u1[1:length(airy_v_GSBSPG)] .+= airy_v_GSBSPG
    accuracy[i, 2] = Chebyshev_L2error(u1, ue, nvec[end])

    # GSBSPG (numerical integration)
    L2, fc2 = GSBSPG_Chebyshev_NI(T, airy_coeffs_GSBSPG, airy_Wtrial, airy_v_GSBSPG, copy(fc))
    u2 = airy_Wtrial * (L2 \ fc2)
    u2[1:length(airy_v_GSBSPG)] .+= airy_v_GSBSPG
    accuracy[i, 3] = Chebyshev_L2error(u2, ue, nvec[end])

    # ultraspherical spectral method
    airy_coeffs_ultraS, airy_bc, airy_bcvals = airy_ultraS(T, n)
    u3 = ultraSsolve(airy_coeffs_ultraS, airy_bc, airy_bcvals, f)
    accuracy[i, 4] = Chebyshev_L2error(u3, ue, nvec[end])
end
