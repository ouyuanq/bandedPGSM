...
function Wtlmul!(W::BandedMatrix{T}, A::BandedMatrix{T}, Aunow::Integer) where T
    ...
    Wl = W.l
    Wdata, szW1 = W.data, size(W, 1)
    Adata, Al, Au = A.data, A.l, A.u
    szAdata1, szA1 = size(Adata, 1), size(A, 1)
    Aunow = Aunow + Wl
    @inbounds for j in axes(Adata, 2)
        Aj = view(Adata, Au+1-min(Aunow, j-1):min(szAdata1, Au+1+szA1-j), j)
        Wind = max(1, j-Aunow):min(szA1, j+Al)
        for i = 1:length(Aj)+min(0, szW1-Wl-j) # the last Wl columns can not be fully computed
            Windi = Wind[i]
            Aj[i] = Aj[i] * inbands_getindex(Wdata, 0, Windi, Windi)
            for k = i+1:i+Wl
                Wik = inbands_getindex(Wdata, 0, Wind[k], Windi)
                Aj[i] = muladd(Wik, Aj[k], Aj[i])
            end
        end
    end

    # discard the last N rows
    _BandedMatrix(Adata, Base.OneTo(size(W, 2)), A.l, A.u)
end
